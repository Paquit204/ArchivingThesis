...
if ($thesis_table_exists) {
    $pending_stmt = $conn->prepare("SELECT COUNT(*) as count FROM thesis_table WHERE student_id = ? AND (is_archived = 0 OR is_archived IS NULL)");
...
if (isset($_GET['accept_invite']) && isset($_GET['thesis_id'])) {
    $notif_id = intval($_GET['accept_invite']);
    $update_stmt = $conn->prepare("UPDATE notifications SET is_read = 1 WHERE notification_id = ? AND user_id = ?");
    $update_stmt->bind_param("ii", $notif_id, $user_id);
    $update_stmt->execute();
    $update_stmt->close();
    header("Location: student_dashboard.php?view=notifications&msg=accepted");
    exit;
}

if (isset($_GET['decline_invite']) && isset($_GET['thesis_id'])) {
    $notif_id = intval($_GET['decline_invite']);
    $update_stmt = $conn->prepare("UPDATE notifications SET is_read = 1 WHERE notification_id = ? AND user_id = ?");
    $update_stmt->bind_param("ii", $notif_id, $user_id);
    $update_stmt->execute();
    $update_stmt->close();
    header("Location: student_dashboard.php?view=notifications&msg=declined");
    exit;
}
...
$show_msg = isset($_GET['msg']) ? $_GET['msg'] : '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .notification-header { padding: 12px 16px; border-bottom: 1px solid #eee; display: flex; justify-content: space-between; align-items: center; }
        .notification-header h4 { font-size: 0.9rem; font-weight: 600; color: #732529; }
        body.dark-mode .notification-header h4 { color: #FE4853; }
        .notification-header a { font-size: 0.7rem; color: #FE4853; cursor: pointer; text-decoration: none; }
        ...
        .notification-footer { padding: 10px 16px; border-top: 1px solid #eee; text-align: center; }
        .notification-footer a { font-size: 0.75rem; color: #FE4853; text-decoration: none; }
        ...
        .dropdown-content a { display: flex; align-items: center; gap: 10px; padding: 12px 16px; color: #333; text-decoration: none; transition: background 0.2s; }
        ...
        .chart-header h3 { font-size: 0.9rem; color: #732529; }
        body.dark-mode .chart-header h3 { color: #FE4853; }
        .chart-header select { padding: 0.25rem 0.5rem; border-radius: 6px; border: 1px solid #e0e0e0; background: white; font-size: 0.7rem; }
        body.dark-mode .chart-header select { background: #4a4a4a; border-color: #6E6E6E; color: #e0e0e0; }
        ...
        .feedback-preview { max-width: 300px; }
        .btn-view {
            display: inline-flex;
            align-items: center;
            gap: 0.3rem;
        ...
        .btn-view:hover { background: #732529; transform: translateY(-2px); }
        .view-all-link { display: inline-flex; align-items: center; gap: 0.5rem; margin-top: 1rem; color: #FE4853; text-decoration: none; font-size: 0.8rem; }
        ...
        .mark-all-btn:hover { background: #732529; transform: scale(1.02); }
        .notif-list-page {
            display: flex;
            flex-direction: column;
            gap: 0.75rem;
        ...
        .notif-actions-page { display: flex; gap: 0.75rem; margin-top: 0.75rem; }
        .btn-accept {
            background: #10b981;
            color: white;
            padding: 0.3rem 0.8rem;
        ...
        @media (max-width: 1024px) { .stats-grid { grid-template-columns: repeat(3,1fr); } .charts-section { grid-template-columns: 1fr; } }
        ...
        @media (max-width: 480px) { .stats-grid { grid-template-columns: 1fr; } .chart-container { height: 200px; } }
    </style>
</head>
<body>
...
<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="student_dashboard.php?view=notifications" class="nav-link <?= $current_view == 'notifications' ? 'active' : '' ?>">
            ...
            <?php if($unreadCount > 0 && $current_view != 'notifications'): ?>
            ...
            <div class="user-info">
                <div class="notification-container">
                    ...
                    <div class="notification-dropdown" id="notificationDropdown">
                        <div class="notification-header"><h4>Recent Notifications</h4><a href="student_dashboard.php?view=notifications">View all notifications</a></div>
                        <div class="notification-list">
                            ...
                            <?php else: foreach($recentNotifications as $notif): ?>
                                <div class="notification-item <?= $notif['is_read']==0 ? 'unread' : '' ?>" data-notification-id="<?= $notif['notification_id'] ?>" data-thesis-id="<?= $notif['thesis_id'] ?? 0 ?>">
                                    <div class="notif-message"><i class="fas fa-bell"></i> <?= htmlspecialchars(substr($notif['message'] ?? '', 0, 80)) ?></div>
                                    <?php if(!empty($notif['thesis_title'])): ?><div class="notif-thesis"><i class="fas fa-book"></i> <?= htmlspecialchars($notif['thesis_title']) ?></div><?php endif; ?>
                ...
                <div class="avatar-dropdown"><div class="avatar" id="avatarBtn"><?= htmlspecialchars($initials) ?></div><div class="dropdown-content" id="dropdownMenu"><a href="profile.php"><i class="fas fa-user-circle"></i> Profile</a><a href="settings.php"><i class="fas fa-cog"></i> Settings</a><hr><a href="/ArchivingThesis/authentication/logout.php"><i class="fas fa-sign-out-alt"></i> Logout</a></div></div>
            </div>
        </header>

        <?php if($show_msg == 'accepted'): ?>
            <div class="alert-msg"><i class="fas fa-check-circle"></i> Invitation accepted successfully!</div>
        <?php elseif($show_msg == 'declined'): ?>
            <div class="alert-msg" style="background:#ef4444;"><i class="fas fa-times-circle"></i> Invitation declined.</div>
        <?php endif; ?>

        <?php if($current_view == 'dashboard'): ?>
            <!-- DASHBOARD VIEW -->
            <div class="welcome-section"><h2>Welcome, <?= htmlspecialchars($first_name) ?>!</h2><p>Here's an overview of your thesis submissions. Member since <?= $member_since ?></p></div>
            ...
            <div class="charts-section">
                <div class="chart-card"><div class="chart-header"><h3>Project Status Distribution</h3><select id="chartPeriod"><option>All Time</option><option>This Semester</option><option>This Year</option></select></div><div class="chart-container"><canvas id="projectStatusChart"></canvas></div></div>
                <div class="chart-card"><div class="chart-header"><h3>Submission Timeline</h3><select id="timelinePeriod"><option>Last 6 Months</option><option>Last Year</option></select></div><div class="chart-container"><canvas id="timelineChart"></canvas></div></div>
            </div>
            <?php if(!empty($recentFeedback)): ?>
            <div class="recent-feedback"><h3><i class="fas fa-comments"></i> Recent Feedback</h3><div class="table-responsive"><table class="feedback-table"><thead><tr><th>TITLE</th><th>FROM</th><th>FEEDBACK</th><th>DATE</th><th>ACTION</th></tr></thead><tbody><?php foreach($recentFeedback as $fb): ?><tr><td><?= htmlspecialchars($fb['thesis_title']) ?></td><td><?= htmlspecialchars($fb['faculty_first'].' '.$fb['faculty_last']) ?></td><td class="feedback-preview"><?= htmlspecialchars(substr($fb['feedback_text'],0,80)) ?>...</td><td><?= date('M d, Y', strtotime($fb['created_at'])) ?></td><td><a href="projects.php?thesis_id=<?= $fb['thesis_id'] ?>" class="btn-view"><i class="fas fa-eye"></i> View</a></td></tr><?php endforeach; ?></tbody></table></div></div>
            <?php endif; ?>
        <?php else: ?>
            ...
            <div class="notifications-page">
                ...
                <div class="notif-list-page">
                    <?php if(empty($allNotifications)): ?>
                        <div class="empty-state-page"><i class="fas fa-bell-slash"></i><p>No notifications yet.</p><p style="font-size:0.8rem;">When you receive notifications, they will appear here.</p></div>
                    <?php else: ?>
                        <?php foreach($allNotifications as $notif): ?>
                            <div class="notif-card <?= $notif['is_read'] == 0 ? 'unread' : '' ?>">
                                ...
                                <div class="notif-content-page">
                                    ...
                                    <?php if(!empty($notif['thesis_title'])): ?><div class="notif-thesis-page"><i class="fas fa-book"></i> <?= htmlspecialchars($notif['thesis_title']) ?></div><?php endif; ?>
...
if(avatarBtn && dropdownMenu) avatarBtn.addEventListener('click', (e) => { e.stopPropagation(); dropdownMenu.classList.toggle('show'); if(notificationDropdown) notificationDropdown.classList.remove('show'); });
if(notificationBell && notificationDropdown) notificationBell.addEventListener('click', (e) => { e.stopPropagation(); notificationDropdown.classList.toggle('show'); if(dropdownMenu) dropdownMenu.classList.remove('show'); });
document.addEventListener('click', (e) => { if(dropdownMenu && !avatarBtn?.contains(e.target)) dropdownMenu.classList.remove('show'); if(notificationDropdown && !notificationBell?.contains(e.target)) notificationDropdown.classList.remove('show'); });
...
document.querySelectorAll('.notification-item').forEach(item => {
    item.addEventListener('click', function(e) {
        const notifId = this.dataset.notificationId, thesisId = this.dataset.thesisId;
        if(notifId && this.classList.contains('unread')) markAsRead(this, notifId);
        if(thesisId && parseInt(thesisId) > 0) setTimeout(() => window.location.href = 'view_project.php?id=' + thesisId, 300);
        else if(notifId && !thesisId) e.stopPropagation();
    });
});
...
if(document.getElementById('projectStatusChart')) {
    ...
    function initCharts() {
        ...
        statusChart = new Chart(document.getElementById('projectStatusChart'), {
            type: 'doughnut',
            data: { labels: ['Pending','Approved','Rejected','Archived'], datasets: [{ data: [chartData.pending,chartData.approved,chartData.rejected,chartData.archived], backgroundColor: ['#f59e0b','#10b981','#ef4444','#6b7280'], borderWidth: 0, cutout: '60%' }] },
            options: { responsive: true, plugins: { legend: { position: 'bottom', labels: { color: textColor } }, tooltip: { callbacks: { label: (ctx) => { const val = ctx.raw; const total = ctx.dataset.data.reduce((a,b)=>a+b,0); const pct = total>0 ? Math.round((val/total)*100) : 0; return `${ctx.label}: ${val} (${pct}%)`; } } } } }
        });
        timelineChart = new Chart(document.getElementById('timelineChart'), {
            type: 'line',
            data: { labels: ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'], datasets: [{ label: 'Submissions', data: [2,3,5,4,6,8,7,9,5,4,3,2], borderColor: '#FE4853', backgroundColor: 'rgba(254,72,83,0.1)', borderWidth: 3, fill: true, tension: 0.4 }] },
            options: { responsive: true, scales: { y: { beginAtZero: true, grid: { color: gridColor }, ticks: { stepSize: 1, color: textColor } }, x: { ticks: { color: textColor } } }, plugins: { legend: { labels: { color: textColor } } } }
        });
    }
    initCharts();
}
