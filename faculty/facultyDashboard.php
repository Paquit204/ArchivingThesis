...
$recentNotifications = [];
$notif_list_query = "SELECT notification_id as id, user_id, thesis_id, message, type, link, status, created_at 
                     FROM notifications 
                     WHERE user_id = ? 
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-card { background: white; border-radius: 20px; padding: 24px; display: flex; align-items: center; gap: 20px; border: 1px solid #fee2e2; }
        ...
        .filter-btn { padding: 6px 12px; background: #fef2f2; color: #dc2626; text-decoration: none; border-radius: 20px; font-size: 0.75rem; font-weight: 500; }
        ...
        .btn-review-small, .btn-view-small { display: inline-flex; align-items: center; gap: 5px; padding: 5px 12px; text-decoration: none; border-radius: 20px; font-size: 0.7rem; }
        .btn-review-small { background: #dc2626; color: white; }
        .btn-view-small { background: #dc2626; color: white; }
                    ...
                    <div class="notification-list" id="notificationList">
                        ...
                        <?php else: ?>
                            <?php foreach ($recentNotifications as $notif): ?>
                                <div class="notification-item <?= $notif['status'] == 0 ? 'unread' : '' ?>" data-id="<?= $notif['id'] ?>" data-thesis-id="<?= $notif['thesis_id'] ?>">
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <div class="nav-menu">
            ...
            <a href="reviewThesis.php" class="nav-item"><i class="fas fa-file-alt"></i><span>Review Theses</span>
                <?php if ($pendingCount > 0): ?><span class="notification-badge-sidebar"><?= $pendingCount ?></span><?php endif; ?>
            </a>
            <a href="notification.php" class="nav-item"><i class="fas fa-bell"></i><span>Notifications</span>
                <?php if ($notificationCount > 0): ?><span class="notification-badge-sidebar"><?= $notificationCount ?></span><?php endif; ?>
            </a>
        </div>
            ...
            <?php else: ?>
                <div class="theses-list">
                    ...
                    <div class="thesis-item">
                        ...
                        <a href="reviewThesis.php?id=<?= $thesis['thesis_id'] ?>" class="review-btn">Review</i></a>
                    </div>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
                    ...
                    <table class="theses-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><span class="status-badge status-<?= $submission['status'] ?>"><?= ucfirst($submission['status']) ?></span></td>
                                <td>
